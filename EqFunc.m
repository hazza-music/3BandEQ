function [h_low,h_mid,h_high,h_eq,w,b,a] = EqFunc(fs,G_low,G_mid,G_high)
...
[b_low,a_low] = ShelvingFilter(fs,G_low,'Low');
[h_low,~] = freqz(b_low,a_low,2048);

% Calculating filter coefficients and response for mid peak
[b_mid,a_mid] = PeakingFilter(fs,G_mid);
[h_mid,~] = freqz(b_mid,a_mid,2048);

% Calculating filter coefficients and response for high shelf
[b_high,a_high] = ShelvingFilter(fs,G_high,'High');
[h_high,~] = freqz(b_high,a_high,2048);

% Calculating created cascaded equalizing filter response and coefficients
h_eq = h_low .* h_mid .* h_high;
